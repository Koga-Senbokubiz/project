...
class BBORDXMLtoDFAX:
    ...
    def _load_excel_defs(self) -> None:
        try:
            self.logger.info("Excel定義読込開始: %s", self.def_xlsx)
    ...
    def do_tran(self, infile: str | Path) -> tuple[bool, str]:
        try:
            self.logger.info("XML解析開始: %s", infile)
            root = ET.parse(str(infile)).getroot()
            orders = self._parse_orders(root)
            self.logger.info("受注件数: %s file=%s", len(orders), infile)
            for idx, order in enumerate(orders, start=1):
                self.logger.info("受注処理開始 %s/%s trade_number=%s", idx, len(orders), order.trade_number)
                self._out04(order)
                self._out05(order)
                self.logger.info("受注処理完了 %s/%s trade_number=%s", idx, len(orders), order.trade_number)
