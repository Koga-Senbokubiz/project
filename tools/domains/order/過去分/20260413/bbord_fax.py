...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--ini", default=str(Path(__file__).with_name(DEFAULT_INI_NAME)), help="ini file path")
...
def load_settings(ini_path: str | Path) -> dict[str, str]:
    ...
    missing = [k for k in ("log_dir", "snd_id", "bb_fax_no", "debug_mode", "debug_fax_no") if not settings[k]]
    if missing:
        raise KeyError(f"missing required ini keys in [{DEFAULT_SECTION}]: {', '.join(missing)}")
    return settings


def configure_logger(jobid: str, log_dir: str | Path, log_level: str = "INFO") -> logging.Logger:
    base_dir = Path(log_dir)
    job_dir = base_dir / jobid
    job_dir.mkdir(parents=True, exist_ok=True)
    log_file = job_dir / "convert-main.log"
...
def validate_settings(settings: dict[str, str]) -> None:
    if len(settings["snd_id"]) != 6 or not settings["snd_id"].isdigit():
        raise SystemExit(f"HAISIN:'{settings['snd_id']}' is wrong!")
    if len(settings["bb_fax_no"]) != 10 or not settings["bb_fax_no"].isdigit():
        raise SystemExit(f"BBFAX:'{settings['bb_fax_no']}' is wrong!")
    if len(settings["debug_fax_no"]) != 10 or not settings["debug_fax_no"].isdigit():
        raise SystemExit(f"DEBUG FAX:'{settings['debug_fax_no']}' is wrong!")
    if settings["debug_mode"] not in {"0", "Y"}:
        raise SystemExit(f"DEBUG MODE:'{settings['debug_mode']}' is wrong! expected 0 or Y")


def main(argv: Sequence[str] | None = None) -> int:
    args = parse_args(argv)
    settings = load_settings(args.ini)
    validate_settings(settings)

    logger = configure_logger(args.jobid, settings["log_dir"], settings["log_level"])
    logger.info("D-FAX変換開始 jobid=%s ini=%s", args.jobid, args.ini)
    ...
    try:
        ...
        if not input_dir.exists() or not input_dir.is_dir():
            raise FileNotFoundError(f"input_dir not found or not directory: {input_dir}")

        targets = sorted([p for p in input_dir.iterdir() if p.is_file() and p.stat().st_size > 0])
        logger.info("入力対象件数=%s", len(targets))
        if not targets:
        ...
        logger.info("D-FAX変換終了 jobid=%s", args.jobid)
        return 0
    except Exception:
        logger.exception("D-FAX変換異常終了 jobid=%s", args.jobid)
        raise
